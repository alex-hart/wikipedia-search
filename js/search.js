$(document).ready(function() {
  ...
  $idForm.submit(function(e){
    ...
    $.getJSON("https://en.wikipedia.org/w/api.php?action=opensearch&datatype=json&limit=5&search="
    + searchQuery + "&callback=?", function(data) {
      //Updating HTML of webpage for every search result
      for (i = 0; i < data[1].length; i++) {
        html += "<a class='article-links' href='" + data[3][i] +
        "' target='blank'> <div class='article-result'> <h1>" + data[1][i] +
        "</h1> <p>" + data[2][i] + "</p> </div> </a>"
      }
